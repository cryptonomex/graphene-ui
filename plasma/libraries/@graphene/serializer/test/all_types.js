...
let allTypes = {
    ...
    static_variant: [
        [1, {account_id: "1.2.2", name: "abc"}],
        [1, {account_id: "1.2.1", name: "abc"}],
        [1, {account_id: "1.2.3", name: "abc"}],
...
describe("all types", ()=> {
    ...
    it("to buffer", ()=>{
        ...
        assert.equal(
            toBuffer( toObject(allTypes) ).toString("hex"), // serializable
            toBuffer( fromObject( allTypes )).toString("hex"), // non-serializable
            "serializable and non-serializable"
        )
    })
    ...
    // keep last
    it("visual check", ()=> {
        console.log(JSON.stringify(toObject(fromObject(allTypes)), null, 4))
    })
